...
public class ShootRedPlay4 : MonoBehaviour {
	...
	void Update () {
		...
		if (GetComponent<MovRedPlay4> ().isFilled == true) {
			fill = true;
		}
		...
		if (GetComponent<MovRedPlay4> ().faceLeft == true) {
			facingLeft = true;
			facingRight = false;
		}else if (GetComponent<MovRedPlay4> ().faceRight == true) {
			facingRight = true;
			facingLeft = false;
		}
		...
		if (GetComponent<MovRedPlay4> ().balloonPop == false && isShooting == true) {
		...
		if (Input.GetKeyUp ("7") || Input.GetKeyUp ("6") && isShooting == true) {
			//GetComponent<Animator>().SetInteger("State",0);
			isShooting = false;
		}
		
		if (Input.GetKeyUp ("7") || Input.GetKeyUp ("6")) {
			GetComponent<Animator>().SetInteger("State",0);
			timePress = 0;
			fill = false;
			//GetComponent<MovRedPlay4> ().balloonPop = true;
		}
		
		
		
		if ((Input.GetKey ("7") || Input.GetKey ("6")) && fill == true)  {
			
			isShooting = true;
			fill = false;
			GetComponent<MovRedPlay4> ().balloonPop = false;
			
			if (fill == false){
				GetComponent<MovRedPlay4> ().isFilled = false;
			}
		...
		if (Input.GetKeyUp ("7") && fill == false) {
		...
		if (Input.GetKeyUp ("6") && fill == false) {
